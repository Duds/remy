# Remy Bug Report — Archived (2026-03-11)

Archived bugs 1–12, 42–45 (all fixed). See `BUGS.md` for active bugs.
...
## Bug 42: Docker container log timestamps reported in UTC, not local time (AEDT)

- **Symptom:** When Remy reports her session start time from logs (e.g. during `/diagnostics` or a health check), she states the time in UTC (e.g. "09:20") rather than local Canberra time (AEDT, UTC+11). This caused Remy to incorrectly report a session start of "09:20 this morning" when the actual local time was "20:20 tonight" — an 11-hour error. The discrepancy is only detectable if the user cross-references with the current time.
- **User flow:** User asked Remy to run diagnostics → Remy read log timestamps → reported session start as "09:20 this morning" → user pointed out they had just restarted Docker → investigation revealed logs are in UTC; 09:20 UTC = 20:20 AEDT.
- **Root cause:** The Docker container does not have `TZ` set to `Australia/Canberra`. Python's logging (and likely the container's system clock output) defaults to UTC. Log timestamps are therefore in UTC, not the user's local timezone. When Remy reads log lines and reports times to the user without converting to local time, the times appear wrong.
- **Impact:** Medium. Misleads the user about when the session started, when errors occurred, and when scheduled jobs fired. Erodes trust in diagnostic reporting. Worse in AEDT (UTC+11) — times can be off by 10–11 hours.
- **Priority:** Medium
- **Status:** ✅ Fixed
- **Location:**
  - `docker-compose.yml` or `Dockerfile` — `TZ` environment variable not set
  - `remy/utils/logging.py` (or wherever log format/handlers are configured) — formatter does not specify local timezone
  - `remy/ai/tools/logs.py` (or equivalent) — `get_logs` tool returns raw log lines without timezone conversion
- **Fix:** Set `TZ=${TZ:-Australia/Canberra}` in `docker-compose.yml` under `environment:` for the Remy service so the container uses local time for logs. Default in Dockerfile remains `TZ=UTC`; compose override provides Australia/Canberra when unset.
- **Reported:** 2026-03-10 (Dale Rogers — session start time discrepancy noticed during health check)
- **Fixed:** 2026-03-10

---

## Bug 43: Missing space when stitching message threads together (Bug 8 regression?)

- **Symptom:** When assembling/stitching message threads (e.g. multi-part responses, tool-turn continuations, or multi-block content), a space is missing between segments. Adjacent text blocks appear concatenated without a separator (e.g. "helloworld" instead of "hello world").
- **Context:** Bug 8 (Fixed 2026-03-05) addressed spaces being pruned when assembling chat transcripts from `.jsonl` content blocks, and switched from `" ".join(...)` to `"".join(...)` in `_extract_text`. It's possible that fix over-corrected (removing all joining spaces), or that the thread-stitching path is a different code path not covered by Bug 8's fix.
- **Root cause:** Unknown — likely one of:
  1. `"".join(...)` fix from Bug 8 removed a necessary space between adjacent blocks in a different code path
  2. Thread continuation logic (where a follow-up tool-turn or streamed chunk is appended to prior text) does not insert a space between the end of the previous segment and the start of the new one
  3. A `.strip()` call on individual segments removes a trailing/leading space that serves as the separator
- **Impact:** Medium. Readability of responses; possible incorrect prompts/context if spacing affects parsing or meaning.
- **Priority:** Medium
- **Status:** ✅ Fixed
- **Location:** `remy/bot/handlers/base.py` — `_sanitize_messages_for_claude` and new `_join_content_parts`.
- **Fix:** Added `_join_content_parts(parts)` that joins content block parts and inserts a space between two adjacent non-empty segments when the first does not end with whitespace and the second does not start with whitespace, so "hello" + "world" becomes "hello world". Bug 8 behaviour (preserving empty and space-only segments) unchanged.
- **Related:** Bug 8 (spaces pruned in chat transcript assembly — may be a regression of that fix)
- **Reported:** 2026-03-10 (Dale Rogers)
- **Fixed:** 2026-03-10

---

## Bug 44: Board replies not formatted with Telegram MarkdownV2

- **Symptom:** Replies generated by the Board of Directors (via `run_board`) are delivered to the user without appropriate Telegram MarkdownV2 formatting. Headers, bold text, bullet points, and other structural elements appear as raw markdown syntax or plain text rather than rendered formatting.
- **Root cause:** Unknown — likely the board reply is returned as a raw string and passed directly to Telegram without being processed through the MarkdownV2 formatting/escape pipeline used for normal chat responses.
- **Impact:** Medium. Board responses are typically long and structured; without formatting they are hard to read. Defeats the purpose of a rich multi-agent analysis output.
- **Priority:** Medium
- **Status:** ✅ Fixed
- **Location:** `remy/agents/background.py` — board (and other background task) results are sent via `BackgroundTaskRunner.run()`.
- **Fix:** Board replies were already sent with `format_telegram_message` and `parse_mode="MarkdownV2"`. When Telegram returns BadRequest for entity parse (e.g. unescaped characters in report content), the runner now catches it via `is_entity_parse_error`, logs at DEBUG, and retries with `parse_mode="Markdown"` so basic bold/italic still render; if that fails, falls back to plain text.
- **Reported:** 2026-03-10 (Dale Rogers)
- **Fixed:** 2026-03-10

---

## Bug 45: Morning briefing echoed stale emotional/sobriety phrasing ("in the clear")

- **Symptom:** Morning briefing Telegram message sometimes included incorrect or stale emotional context, e.g. sobriety milestone described as "in the clear" when it shouldn't have been. User reported the briefing "marked own is in the clear" incorrectly.
- **Root cause:** The proactive pipeline for morning briefing loaded recent conversation turns and passed them to Claude. When composing the briefing, the model had access to prior messages that contained emotional or sobriety-related phrasing and could echo or blend that into the briefing, so stale context leaked in.
- **Impact:** Medium. Briefing felt wrong or confusing; sobriety/emotional framing should not appear in the morning summary unless it comes from the structured context (e.g. evening check-in has counters; morning briefing does not).
- **Priority:** Medium
- **Status:** ✅ Fixed
- **Location:** `remy/bot/pipeline.py` — `run_proactive_trigger()` built message history for all proactive triggers including morning briefing.
- **Fix:** For scenario `"briefing"` only, use empty conversation history when building messages. The morning briefing is now composed solely from the structured context JSON (goals, calendar, etc.); no prior turns are passed, so no stale emotional or sobriety phrasing can leak in. Evening check-in and other proactive triggers still receive recent turns as before.
- **Reported:** 2026-03-05 (TODO.md); documented 2026-03-10
- **Fixed:** 2026-03-10
