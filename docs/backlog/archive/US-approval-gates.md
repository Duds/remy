# User Story: Approval Gates for Bulk / High-Stakes Actions

**Status:** ✅ Done
**Priority:** ⭐⭐⭐ High
**Effort:** Medium
**Source:** [docs/ideas.md §6](../ideas.md)
...
A typical approval gate pattern: agent POSTs an approval request → waits for approval/reject → continues with the approval result.
...
1. **Gate triggers.** The following actions require approval before execution:
   ...
   - Any `gmail_delete` task from an external source (always)
...
7. **Task integration.** For `gmail_delete` tasks from external sources, after Dale confirms, Remy marks the task done with the result. On cancel, marks as needing clarification with note "Dale declined the bulk delete."
...
| Gmail tool executor | Route gmail_delete tasks through approval |
...
| `gmail_delete` task from external source (any size) | Gate always shown; result feeds back to task status |
...
- Board member approval (Remy is single-user)
- Approval logs persisted to notes (nice-to-have, not blocking)
