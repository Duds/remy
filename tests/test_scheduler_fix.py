...
async def test_run_automation_deletes_onetime_before_sending():
    ...
    scheduler = ProactiveScheduler(
        ...
        fact_store=None,  # Avoid _log_completed_reminder await on MagicMock
    ...
    with patch("remy.scheduler.proactive._read_primary_chat_id", return_value=12345):
        with patch.object(scheduler, "_send_reminder", side_effect=mock_send):
            automation = {
                "id": 123,
                "user_id": 1,
                "label": "Remember milk",
    ...
    assert call_order.index("delete") < call_order.index("send"), (
        "delete MUST be called before send"
    )
