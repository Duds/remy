        ...
        try:
            ...
            if _extraction_runner is not None:
                ...
                if _extraction_runner.active_count >= 4:
                    warnings.append(
                        f"Extraction runner near capacity ({_extraction_runner.active_count}/5)"
                    )

            # Check per-user extraction runner
            if _per_user_extraction_runner is not None:
                details["per_user_extraction_active"] = (
                    _per_user_extraction_runner.active_count
                )
            ...
            users_at_limit = [
                uid
                for uid, count in _user_active_requests.items()
            ...
            async with self._db.get_connection() as conn:
                ...
                if self._settings:
                    daily_budget = getattr(
                        self._settings, "max_cost_per_user_per_day_usd", 10.0
                    )
                ...
                message = (
                    f"{call_count} calls, {token_str} tokens, ~${estimated_cost:.2f}"
                )
...
def format_diagnostics_output(
    result: DiagnosticsResult, timezone_str: str = "Australia/Sydney"
) -> str:
    """
